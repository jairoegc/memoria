...
El planeta tierra es constantemente bombardeado por rayos cósmicos provenientes del espacio exterior, correspondiendo principalmente a partículas cargadas como protones y núcleos atómicos. El \sgcnote{dangling modifier} origen de estos rayos cósmicos es variado, y aunque la fuente de algunos es desconocida, la mayor parte de ellos provienen de tormentas solares, agujeros negros e incluso de eventos astronómicos asociados al origen del universo\cite{Tanabashi2018ReviewPhysics}. La velocidad alcanzada por estas partículas cósmicas es tan grande que entran en la categoría de partículas de altas energías, alcanzando desde unos cuantos GeV (Giga Electron Volts) en partículas provenientes del sol, hasta más de 1000 TeV (Tera Electron Volts) para rayos originados en centros galácticos y agujeros negros \cite{DeUndergraduatePhysics} \sgcnote{Trata de dar referencias para todos los datos que pones como verdad absoluta y que son relevantes.}.
...
\section{Motivación}
	El ``Sistema de adquisición de datos para detectores de muones'' nace como un requerimiento del CCTVal \sgcnote{ya introdujiste este acronimo.} para aplicaciones de física de partículas en el marco del proyecto ``sTGC Minería'', cuyo nombre se debe al tipo de detector utilizado, llamado "small-strip Thin Gap Chamber".
	...
	Producto de la colaboración existente entre CCTVal y el experimento ATLAS (A Toroidal LHC ApparatuS) en CERN (Conseil Européen pour la Recherche Nucléaire), CCTVal \sgcnote{cual centro?} cuenta con las herramientas y conocimientos necesarios para la fabricación de detectores de muones. Esta colaboración internacional consiste en la actualización de un sector del experimento ATLAS, llamado Small Wheel, cuyos detectores son fabricados solamente en Chile y otros 4 paises\sgcnote{efectivamente son pocos paises? Quizas sea mejor destacar el rol local de la region.}. La Figura \sgcnote{figura en mayuscula cuando te refieras a una figura especifica con numero.} \ref{img:diagrama-atlas} ilustra el diagrama del experimento, indicandose la Small Wheel en el sector central del instrumento. La Figura \ref{img:small-wheel} corresponde a un fotografía  del proceso de montaje de la Small Wheel. Los trapecios dorados (marcado en rojo) \sgcnote{editar la figura para marcar estas partes. No es evidente encontrar los trapezoides. No es un juego de donde esta Wally.} que se observan en dicha imagen corresponden a los detectores sTGC, donde los tercios superiores (marcado en azul) de cada trapecio son los detectores fabricados en CCTVal. La Figura \ref{img:stgc-silab} ilustra una fotografía de los detectores sTGC en su etapa de fabricación. La tecnología utilizada para confeccionar estos detectores es la misma a utilizar en ``sTGC Minería''.
	
	Actualmente, CCTVal \sgcnote{cual centro?} cuenta con el conocimiento para fabricación de detectores, pero no cuenta con toda la experiencia respecto al manejo de datos provenientes de ellos. El diseño de un sistema de adquisición representa uno de los primeros pasos para adquirir experiencia práctica. Esta experiencia será \sgcnote{sera} valiosa para este y futuros proyectos relacionados con detección de partículas.
